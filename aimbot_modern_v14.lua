-- Modern Combat Aimbot V14
-- 此版本与V11相同，但添加了NoCollision开关
...
local ScreenGui, MainFrame, StatusLabel, EnabledButton, EnabledKnob
...
local function CreateUI()
    ...
    CreateToggle("No Collision", function(enabled)
    ...
    CloseButton.MouseButton1Click:Connect(function()
        ...
        -- 显示加载器的按钮
        if getgenv().PuneetButton then
            getgenv().PuneetButton.Visible = true
        end
    end)

    return ScreenGui, MainFrame, StatusLabel, EnabledButton, EnabledKnob
end
...
--// Show UI Function
function Environment.ShowUI()
    if MainFrame then
        MainFrame.Visible = true
        if getgenv().PuneetButton then
            getgenv().PuneetButton.Visible = false
        end
    end
end
...
--// Main Loop
local function Load()
    ScreenGui, MainFrame, StatusLabel, EnabledButton, EnabledKnob = CreateUI()
    ...
    ServiceConnections.RenderSteppedConnection = RunService.RenderStepped:Connect(function()
        ...
        -- NoCollision
        if Environment.Settings.NoCollision and LocalPlayer.Character then
            for _, part in pairs(LocalPlayer.Character:GetDescendants()) do
                if part:IsA("BasePart") then
...
function Environment.Functions:Exit()
    ...
    if game.CoreGui:FindFirstChild("AimbotUI") then
        game.CoreGui.AimbotUI:Destroy()
    end
...
print("NEWAIMBOT V14 loaded successfully!")
